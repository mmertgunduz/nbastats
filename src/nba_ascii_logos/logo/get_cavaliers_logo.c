/**
 * @file get_cavaliers_logo.c
 * 
 * @author Mehmet Mert Gunduz (mmertgunduz@protonmail.com)
 * @brief Draw the Cleveland Cavaliers logo and color it by using ANSI color code.
 * 
 * This function is used to construct a colored ASCII representation of Cleveland Cavaliers (CLE) team logo.
...
// Cleveland Cavaliers (CLE) logo colors.
#define CLE_GOLD    "\e[38;2;198;151;31m" // Hex: #C6971F
#define CLE_WINE    "\e[38;2;134;31;65m"  // Hex: #861F41

/**
 * @brief Get the Cleveland Cavaliers logo in ASCII format and makes it colorful.
 * 
 * @return char* Cleveland Cavaliers (CLE) logo in colored ASCII format.
 */
char *get_cavaliers_logo() {
    char cavaliers_logo[] = 
"                         ==============\n\
                   ===++###############*==\n\
               ===+#########*+++########*==\n\
...
====+############*==========\n\
  ======================\n\
    ...
    for (int i = 0; i < len; i++) {
        ...
        c[0] = cavaliers_logo[i];
        c[1] = '\0';
        
        if (c[0] == '=') {
            add_colorful_char(colorful_logo, c, CLE_GOLD);
        } else {
            add_colorful_char(colorful_logo, c, CLE_WINE);
        }
    }
