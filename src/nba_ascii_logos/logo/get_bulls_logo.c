/**
 * @file get_bulls_logo.c
 * 
 * @author Mehmet Mert Gunduz (mmertgunduz@protonmail.com)
 * @brief Draw the Chicago Bulls logo and color it by using ANSI color code.
 * 
 * This function is used to construct a colored ASCII representation of Chicago Bulls (CHI) team logo.
...
// Chicago Bulls (CHI) logo colors.
#define CHI_RED    "\e[38;2;206;17;65m"   // Hex: #CE1141
#define CHI_BLACK  "\e[38;2;60;60;60m"    // Hex: #3C3C3C

/**
 * @brief Get the Chicago Bulls logo in ASCII format and makes it colorful.
 * 
 * @return char* Chicago Bulls (CHI) logo in colored ASCII format.
 */
char *get_bulls_logo() {
    char bulls_logo[] = 
"    @                                       @@\n\
 ...
 ::   :**%@@@@@%%%%%%%%%%%%%%%%%%%@@@@@%#*:   ::\n\
   ...
   *. .      +*********************+    .  ..*\n\
    ...
    for (int i = 0; i < len; i++) {
        ...
        if (c[0] == '@') {
            add_colorful_char(colorful_logo, c, CHI_BLACK);
        } else {
            add_colorful_char(colorful_logo, c, CHI_RED);
        }
    }
