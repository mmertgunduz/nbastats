/**
 * @file get_pacers_logo.c
 * 
 * @author Mehmet Mert Gunduz (mmertgunduz@protonmail.com)
 * @brief Draw the Indiana Pacers logo and color it by using ANSI color code.
 * 
 * This function is used to construct a colored ASCII representation of Indiana Pacers (IND) team logo.
...
// Indiana Pacers (IND) logo colors.
#define IND_NAVY   "\e[38;2;0;45;98m"     // Hex: #002D62
#define IND_YELLOW "\e[38;2;253;187;48m"  // Hex: #FDBB30

/**
 * @brief Get the Indiana Pacers logo in ASCII format and makes it colorful.
 * 
 * @return char* Indiana Pacers (IND) logo in colored ASCII format.
 */
char *get_pacers_logo() {
    char pacers_logo[] = 
"        -=+++++++++++++++++++=----\n\
        -*%%%%%%%%%%%%%%%%%%%%%%%%#=--\n\
       --%%%%%%%%%%%%%%*+--=*#++*%%%%+--\n\
...
-++++++++++++++=-";
    ...
    for (int i = 0; i < len; i++) {
        ...
        if (c[0] == '-') {
            add_colorful_char(colorful_logo, c, IND_YELLOW);
        } else {
            add_colorful_char(colorful_logo, c, IND_NAVY);
        }
    }
