/**
 * @file get_heat_logo.c
 * 
 * @author Mehmet Mert Gunduz (mmertgunduz@protonmail.com)
 * @brief Draw the Miami Heat logo and color it by using ANSI color code.
 * 
 * This function is used to construct a colored ASCII representation of Miami Heat (MIA) team logo.
...
// Miami Heat (MIA) logo colors.
#define MIA_RED     "\e[38;2;152;0;46m"    // Hex: #98002E
#define MIA_YELLOW  "\e[38;2;249;160;27m"  // Hex: #F9A01B
#define MIA_BLACK   "\e[38;2;6;25;34m"     // Hex: #061922
#define MIA_WHITE   "\e[38;2;255;255;255m" // Hex: #FFFFFF

/**
 * @brief Get the Miami Heat logo in ASCII format and makes it colorful.
 * 
 * @return char* Miami Heat (MIA) logo in colored ASCII format.
 */
char *get_heat_logo() {
    char heat_logo[] = 
"                      @#@\n\
  ...
  %:..*@         @%@####%%%%###@     @%: :@\n\
    ...
    /* The variable line counter for setting the yellow fire.  */
    int n = 0;

    /* Length of the colorless logo. */
    int len = strlen(heat_logo);
    ...
    for (int i = 0; i < len; i++) {
        ...
        if (n < 5) {
            add_colorful_char(colorful_logo, c, MIA_YELLOW);
            continue;
        }

        if (c[0] == '@') {
            add_colorful_char(colorful_logo, c, MIA_BLACK);
        } else if (c[0] == '.' || c[0] == ':') {
            add_colorful_char(colorful_logo, c, MIA_WHITE);
        } else {
            add_colorful_char(colorful_logo, c, MIA_RED);
        }
    }
