/**
 * @file get_knicks_logo.c
 * 
 * @author Mehmet Mert Gunduz (mmertgunduz@protonmail.com)
 * @brief Draw the New York Knicks logo and color it by using ANSI color code.
 * 
 * This function is used to construct a colored ASCII representation of New York Knicks (NYK) team logo.
 ...
 * 
 * @return char* New York Knicks (NYK) logo in colored ASCII format.
 */
char *get_knicks_logo() {
    char knicks_logo[] = 
"              ---==+++++++++==---\n\
          --==+*#%##########%%#*+==--\n\
        -==*%%##*+++++++++++++*##%%*==-\n\
  ...
  -=#%*+++*##%%###***********###%%##*+++#%#=-\n\
   -=#%#++++++++*#############*++++++++#%#=-\n\
    ...
    for (int i = 0; i < len; i++) {
        ...
        if (c[0] == '#') {
            add_colorful_char(colorful_logo, c, NYK_BLUE);
        } else {
            add_colorful_char(colorful_logo, c, NYK_ORANGE);
        }
    }
