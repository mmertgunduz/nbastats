/**
 * @file get_rockets_logo.c
 * 
 * @author Mehmet Mert Gunduz (mmertgunduz@protonmail.com)
 * @brief Draw the Houston Rockets logo and color it by using ANSI color code.
 * 
 * This function is used to construct a colored ASCII representation of Houston Rockets (HOU) team logo.
...
// Houston Rockets (HOU) logo colors.
#define HOU_RED    "\e[38;2;206;17;65m"   // Hex: #CE1141 
#define HOU_BLACK  "\e[38;2;60;60;60m"    // Hex: #3C3C3C

/**
 * @brief Get the Houston Rockets logo in ASCII format and makes it colorful.
 * 
 * @return char* Houston Rockets (HOU) logo in colored ASCII format.
 */
char *get_rockets_logo() {
    char rockets_logo[] = 
"          %\n\
          %#%\n\
          %##%%%%%\n\
  ...
  @@@@%%%############%%%%%@@\n\
    ...
    for (int i = 0; i < len; i++) {
        ...
        if (c[0] == '@') {
            ...
            add_colorful_char(colorful_logo, c, HOU_BLACK);
        } else {
            add_colorful_char(colorful_logo, c, HOU_RED);
        }
