/**
 * @file get_hornets_logo.c
 * 
 * @author Mehmet Mert Gunduz (mmertgunduz@protonmail.com)
 * @brief Draw the Charlotte Hornets logo and color it by using ANSI color code.
 * 
 * This function is used to construct a colored ASCII representation of Charlotte Hornets (CHH) team logo.
...
// Charlotte Hornets (CHH) logo colors.
#define CHH_TEAL    "\e[38;2;29;17;96m"    // Hex: #1D1160
#define CHH_PURPLE  "\e[38;2;29;63;114m"   // Hex: #00788C
#define CHH_WHITE   "\e[38;2;255;255;255m" // Hex: #FFFFFF

/**
 * @brief Get the Charlotte Hornets logo in ASCII format and makes it colorful.
 * 
 * @return char* Charlotte Hornets (CHH) logo in colored ASCII format.
 */
char *get_hornets_logo() {
    char hornets_logo[] = 
"***                                      *#+\n\
 *%##*                                *##%*\n\
**#+=*#**                          **#*==#**\n\
  ...
  *#. . .+-.:+#*++*%%##%%*++*#*-.-+. . .**\n\
   *#-.   .:#+.-#%%%****%%%#-.=#-.. ..:#*\n\
    ...
    for (int i = 0; i < len; i++) {
        ...
        if (c[0] == '%') {
            add_colorful_char(colorful_logo, c, CHH_TEAL);
        } else if (c[0] == '.') {
            add_colorful_char(colorful_logo, c, CHH_WHITE);
        } else {
            add_colorful_char(colorful_logo, c, CHH_PURPLE);
        }
    }
